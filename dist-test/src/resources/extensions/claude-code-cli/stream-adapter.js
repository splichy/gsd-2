...
function buildPromptFromContext(context) {
  ...
  parts.push(
    "<tool_context>\nYou are running inside Claude Code. Use these exact tool names \u2014 do not use lowercase or pi-native names:\n- Shell commands: 'Bash' (not 'bash')\n- File operations: 'Read', 'Write', 'Edit', 'Glob', 'Grep' (PascalCase, not lowercase)\n- GSD workflow tools (gsd_exec, gsd_slice_complete, gsd_task_complete, gsd_plan_slice, etc.) are MCP tools \u2014 call them as mcp__gsd-workflow__<tool_name> (e.g. mcp__gsd-workflow__gsd_exec, mcp__gsd-workflow__gsd_slice_complete)\n</tool_context>"
  );
...
function buildSdkOptions(modelId, prompt, overrides, extraOptions = {}) {
  ...
  const workflowAlreadyInProject = workflowServerName !== void 0 && discovered.includes(workflowServerName);
  let filteredMcpServers = workflowAlreadyInProject ? void 0 : mcpServers;
  ...
  const workflowMcpTools = filteredMcpServers ? Object.keys(filteredMcpServers).map((serverName) => `mcp__${serverName}__*`) : !workflowExplicitlyBlocked && workflowServerName ? [`mcp__${workflowServerName}__*`] : [];
